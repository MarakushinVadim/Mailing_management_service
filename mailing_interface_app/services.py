...
def my_job():
    ...
    def send_mailing():
        ...
        mailings = SendingMailSet.objects.filter(
            next_sending_time__lte=current_datetime, is_active=True
        )
            ...
            if mailing.sending_status != "completed":
                ...
                try:
                    server_response = send_mail(
                        ...
                        recipient_list=[
                            client.email for client in mailing.client_service.all()
                        ],
                        fail_silently=False,
                    )
                    SendTry.objects.create(
                        sending_mail=mailing,
                        status="SUCCESS",
